(function () {
  function blockTimeClass(diff) {
    if (diff <= 13) return "text-success";

    if (diff <= 20) return "text-warning";
  ...
  function mainClass(node, bestBlock) {
    ...
    if (!node.active) return "text-gray";
  ...
  Template.registerHelper("nodes", function (uptime) {
    return Nodes.find(
      {},
      { sort: { "stats.block.number": -1, "stats.active": -1 } }
    );
  });
  ...
  Template.registerHelper("timeClass", function (timestamp, active) {
    if (_.isBoolean(active) && !active) return "text-gray";
  ...
  Template.registerHelper("networkHashrateFilter", function (hashes, isMining) {
    ...
    if (!isMining)
      return new Spacebars.SafeString(
        result.toFixed(1) + ' <span class="small-hash">' + unit + "H/s</span>"
      );

    return new Spacebars.SafeString(
      '? <span class="small-hash">' + unit + "KH/s</span>"
    );
  });
  ...
  Template.registerHelper("gasPriceFilter", function (price) {
    ...
    if (price.length < 4) return numeral(price).format("0,0") + " wei";

    if (price.length < 7) return numeral(price / 1000).format("0,0") + " kwei";
    ...
    if (price.length < 19)
      return (
        numeral(price.substr(0, price.length - 15)).format("0,0") + " finney"
      );
  ...
  Template.registerHelper(
    "propagationAvgTimeClass",
    function (propagationAvg, active) {
      if (!active) return "text-gray";

      if (propagationAvg == 0) return "text-info";

      if (propagationAvg < 1000) return "text-success";

      if (propagationAvg < 3000) return "text-warning";

      if (propagationAvg < 7000) return "text-orange";
  ...
  Template.registerHelper("latencyClass", function (active, latency) {
    if (active === false) return "text-danger";
  ...
  Template.registerHelper("upTimeClass", function (uptime, active) {
    if (active === false) return "text-gray";
  ...
  Template.registerHelper("hashrateClass", function (mining, active) {
    if (!mining || !active) return "text-gray";
  ...
  Template.registerHelper("nodeClientClass", function (info, current) {
    if (
      ...
      typeof info.client === "undefined" ||
      typeof info.client === ""
    )
      return "text-danger";

    if (compareVersions(info.client, "<", current)) return "text-danger";
  ...
  Template.registerHelper("consensusClass", function (nodes, bestBlock) {
    ...
    for (var x = 0; x < nodes.length; x++) {
      if (
        nodes[x].stats.block.number === bestBlock.number &&
        nodes[x].stats.block.hash !== bestBlock.hash
      )
        return "danger";

      if (
        bestBlock.number - nodes[x].stats.block.number > 1 &&
        now - bestBlock.received >= 20 * 1000
      )
        status = "orange";

      if (
        bestBlock.number - nodes[x].stats.block.number === 1 &&
        now - bestBlock.received >= 10 * 1000 &&
        status !== "orange"
      )
        status = "warning";
    }
  ...
  Template.registerHelper("geoTooltip", function (node) {
    ...
    if (node.info.net !== "") {
      string =
        "Network: <b>" +
        (typeof node.info.net !== "undefined" ? node.info.net : "-") +
        "</b>";
    ...
    if (node.info.protocol !== "") {
      string =
        "Protocol: <b>" +
        (typeof node.info.protocol !== "undefined" ? node.info.protocol : "-") +
        "</b>";
    ...
    if (node.info.port !== "") {
      string =
        "Port: <b>" +
        (typeof node.info.port !== "undefined" ? node.info.port : "30303") +
        "</b>";
    ...
    if (node.info.client !== "") {
      string =
        "API: <b>" +
        (typeof node.info.client !== "undefined"
          ? node.info.client
          : "<= 0.0.3") +
        "</b>";
    ...
    if (node.info.os !== "") {
      string =
        "OS: <b>" +
        (typeof node.info.os !== "undefined"
          ? node.info.os + " " + node.info.os_v
          : "?") +
        "</b>";
    ...
    if (node.geo !== null) {
      ...
      if (node.geo.city !== "") string += node.geo.city + ", ";
    ...
    if (node.info.contact !== "") {
      string =
        "Contact: <b>" +
        (typeof node.info.contact !== "undefined" ? node.info.contact : "-") +
        "</b>";
  ...
  Template.registerHelper(
    ...
    function (stats, bestBlock) {
      if (!stats) return;

      var ms = stats.propagationAvg;

      if (bestBlock - stats.block.number > 40) return "∞";
  ...
  Template.registerHelper("nodeVersion", function (version) {
    if (typeof version !== "undefined") {
      ...
      if (tmp[2].indexOf("Linux") === 0) tmp[2] = "linux";

      if (tmp[2].indexOf("Darwin") === 0) tmp[2] = "darwin";
  ...
  Template.registerHelper("hashFilter", function (hash) {
    ...
    if (hash.substr(0, 2) === "0x") hash = hash.substr(2, 64);
